...
type APIHandler struct {
	...
	pushTaskQueues      map[string]queue.PushTaskQueue
	pushTaskQueuesMutex sync.Mutex
...
// PushTaskQueue returns a named push task queue working in the context.
func (handler *APIHandler) PushTaskQueue(name string) queue.PushTaskQueue {
	handler.pushTaskQueuesMutex.Lock()
	defer handler.pushTaskQueuesMutex.Unlock()

	q, ok := handler.pushTaskQueues[name]
	if ok {
		return q
	}

	if handler.pushTaskQueues == nil {
		handler.pushTaskQueues = make(map[string]queue.PushTaskQueue)
	}

	q, err := cmnfactory.NewPushTaskQueue(handler.Context(), name)
	if err != nil {
		// TODO: Cause panic by using logging library.
		panic(err)
	}

	handler.pushTaskQueues[name] = q
	return q
}
